...
namespace _05__Datatable_Example_4__Sorting_
{
    internal class Program
    {
        static void Main(string[] args)
        {
            // Sorting in DataTable is slower usually we will take what is faster later
            ...
            // Sort the DataTable by Name in Ascending Order
            EmployeesDataTable.DefaultView.Sort = "Name ASC";
            EmployeesDataTable = EmployeesDataTable.DefaultView.ToTable();

            Console.WriteLine("\nEmployees List Sorted by Name in Ascending Order :\n");
            foreach (DataRow RecordRow in EmployeesDataTable.Rows)
            {
                Console.WriteLine("ID : {0} \t Name : {1} \t Country : {2} \t Salary : {3} \t Date : {4} \t",
                    RecordRow["ID"], RecordRow["Name"], RecordRow["Country"], RecordRow["Salary"], RecordRow["Date"]);
